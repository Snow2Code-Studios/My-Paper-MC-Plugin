...
public class SnoutPouch {
    private final Map<UUID, Inventory> pouches = new HashMap<>();
    private final File file;
    private final FileConfiguration data;

    public SnoutPouch(File dataFolder) {
        this.file = new File(dataFolder, "mouth_snout.yml");
    ...
    private void ensureDataExists(Player player) {
        String path = player.getUniqueId() + ".snout";
    ...
    public void save(Player player) {
        ...
        if ( inv != null ) {
            ...
            data.set(player.getUniqueId() + ".snout", serialized);
            saveFile();
        }
    }
    ...
    public Inventory load(Player player) {
        ...
        int size = 9;
        Inventory inv = Bukkit.createInventory(null, InventoryType.DISPENSER, "Snout Pouch");
        List<String> serialized = data.getStringList(player.getUniqueId() + ".snout");
