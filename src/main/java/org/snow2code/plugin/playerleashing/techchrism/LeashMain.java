package org.snow2code.plugin.playerleashing.techchrism;
...
import org.snow2code.plugin.playerleashing.techchrism.main.*;
...
public class LeashMain implements LeashSystem {
    ...
    public void start()
    {
        plugin.getServer().getPluginManager().registerEvents(new LeashListener(), plugin);
        ...
        plugin.getServer().addRecipe(targetedLeadRecipe);
    }
    ...
    public static void drawVector(Vector vector, Location location, double resolution) {
        ...
        for (int i = 0; i <= steps; i++) {
            ...
            loc.getWorld().spawnParticle(Particle.DUST, loc, 0, new Particle.DustOptions(Color.LIME, 1F));
        }
    }
    ...
    public static void unleashFrom(Player player, Leashable leashedEntity, boolean remove) {
        ...
        SemiFunc.LeashMessage("abandonment", "chat", player, player, 0);
